...
import { CalendarDays, CircleDot, Plus, Sparkles, Trash2, Trophy, Users, Zap } from "lucide-react";
...
type BillingStatus = {
  currentUser: { name: string | null; email: string; canCreateSession: boolean; creditBalance: number; isUnlimited: boolean } | null;
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h1 className="truncate text-[1.05rem] font-black tracking-[0.01em] text-[var(--text)]">RVerse Play</h1>
            </div>
          </div>
        </div>
      </header>
          ...
          <div className="grid gap-5 px-4 py-5 sm:px-5 lg:grid-cols-[1.25fr_0.9fr] lg:items-end">
            <div className="min-w-0">
              <div className="mb-3 flex flex-wrap items-center gap-2">
                <Pill tone="blue">{billing?.currentUser?.canCreateSession ? "Ready" : billing?.currentUser ? "Credits needed" : "Sign in"}</Pill>
              </div>
              <h2 className="max-w-xl text-2xl font-black leading-tight text-[var(--text)] sm:text-[2rem]">
                Manage sessions, courts, and rotations in one place.
              </h2>
            </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[370px_1fr]">
          <div className="space-y-4">
            <Section title="Join Session">
              <JoinSessionForm defaultName={billing?.currentUser?.name || ""} />
            </Section>

            <Section
              title="New Session"
              action={<Pill tone={billing?.currentUser?.canCreateSession ? "green" : "amber"}>{billing?.currentUser?.canCreateSession ? "Ready" : billing?.currentUser ? "Credits needed" : "Sign in"}</Pill>}
            >
              <div className="space-y-4">
                ...
                <Button onClick={createSession} className="w-full" loading={creatingSession}>
                  <Plus size={16} />
                  {billing?.currentUser?.canCreateSession
                    ? billing.currentUser.isUnlimited
                      ? "Create session"
                      : `Create session (${billing.sessionCreateCreditCost} credits)`
                    : billing?.currentUser
                      ? "Top up to create"
                      : "Sign in to create"}
                </Button>
          ...
          <Section title="Session Board" action={<Pill tone="slate">{sessions.filter((session) => session.status === "ACTIVE").length} active</Pill>}>
                ...
                >
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <a href={`/sessions/${session.id}`} className="truncate text-sm font-black text-[var(--text)] hover:text-emerald-700">{session.name}</a>
                      <Pill tone={session.status === "ACTIVE" ? "green" : "slate"}>{session.status === "ACTIVE" ? "Open" : "Ended"}</Pill>
                      <Pill tone={session.skillBalancing ? "green" : "slate"}>{session.skillBalancing ? "Balanced" : "Manual"}</Pill>
                    </div>
                  ...
                  {session.status === "ENDED" ? (
                    <Button variant="danger" className="shrink-0" onClick={() => deleteSession(session.id)} loading={deletingSessionId === session.id}>
                      <Trash2 size={14} />
                      Delete
                    </Button>
                  ) : (
                    <a href={`/sessions/${session.id}`} className="shrink-0 rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700">Open</a>
                  )}
                </div>
              ))}
              {!sessions.length && (
